...
// Helper functions for memorials collection
export async function createMemorial(memorialData) {
  try {
    // Ensure gallery_images is an array if it exists
    const galleryImages = memorialData.galleryImages || [];

    const memorial = await directus.request(createItem('memorials', {
      name: memorialData.name,
      date_info: memorialData.dateInfo,
      description: memorialData.description,
      profile_image: {
        id: memorialData.profileImage
      },
      gallery_images: galleryImages.map(id => ({ id })),
