import React from 'react';
import rightBand3 from '../../public/images/decoration/Right band 3.png';
import rightBand1 from '../../public/images/decoration/Right band 1.png';
import rightBand2 from '../../public/images/decoration/Right band 2.png';
import leftBand1 from '../../public/images/decoration/Left band 1.png';
import leftBand2 from '../../public/images/decoration/Left band 2.png';
export const BandPattern = (): JSX.Element => {
  return (
    // <div style={{"backgroundColor":"transparent","display":"flex","flexDirection":"row","justifyContent":"center","width":"100%"}}>
    //   <div style={{"border":"0px none","height":"64px","width":"1440px"}}>
    //     <div style={{"height":"64px","left":"0","top":"0","width":"1440px"}}>
    //       <div style={{"height":"430px","position":"relative"}}>
    //         <img style={{"height":"129px","left":"101%","position":"absolute","top":"104px","width":"120px"}} alt="Right band" src={rightBand3} />
    //         <img style={{"height":"156px","left":"67%","position":"absolute","top":"67px","width":"600px"}} alt="Right band" src={rightBand2} />
    //         <img style={{"height":"129px","left":"80%","position":"absolute","top":"0","width":"422px"}} alt="Right band" src={rightBand1} />
    //         <img style={{"height":"129px",right:"82%","position":"absolute","top":"208px","width":"387px"}} alt="Left band" src={leftBand1} />
    //         <img style={{"height":"129px",right:"94%","position":"absolute","top":"300px","width":"218px"}} alt="Left band" src={leftBand2} />
    //       </div>
    //     </div>
    //   </div>
    // </div>
    <div style={{width: "150vw", minWidth: "150vw"}} className="uui-heroheader18_background">
      <div className="uui-heroheader18_background-grid">
        <div
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 20 / span 1 / span 20',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89cb-9f76183b"
          className="uui-heroheader18_background-grid-section background-color-primary300-copy"
          style={{
            gridArea: 'span 1 / span 7 / span 1 / span 7',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89cc-9f76183b"
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 5 / span 1 / span 5',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89cd-9f76183b"
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 3 / span 1 / span 3',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89ce-9f76183b"
          className="uui-heroheader18_background-grid-section background-color-primary100"
          style={{
            gridArea: 'span 1 / span 7 / span 1 / span 7',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89cf-9f76183b"
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 7 / span 1 / span 7',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89d0-9f76183b"
          className="uui-heroheader18_background-grid-section background-color-primary200"
          style={{
            gridArea: 'span 1 / span 11 / span 1 / span 11',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89d1-9f76183b"
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 4 / span 1 / span 4',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89d2-9f76183b"
          className="uui-heroheader18_background-grid-section background-color-primary300"
          style={{
            gridArea: 'span 1 / span 7 / span 1 / span 7',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89d3-9f76183b"
          className="uui-heroheader18_background-grid-section"
          style={{
            gridArea: 'span 1 / span 18 / span 1 / span 18',
          }}
        ></div>
        <div
          id="w-node-ccd4ef73-93e0-43e5-014e-dd1e293f89d4-9f76183b"
          className="uui-heroheader18_background-grid-section background-color-primary100"
          style={{
            gridArea: 'span 1 / span 7 / span 1 / span 7',
          }}
        ></div>
      </div>
      <div className="uui-heroheader18_background-grid-mask"></div>
    </div>
  );
};
